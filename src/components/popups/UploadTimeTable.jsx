...
const UploadTimeTable = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { status } = await dispatch(AddTimeTable(formData));
      if (status === 200) {
        addNotification("Time table uploaded", "success", 5000);
      }
    } catch (err) {
      console.log("err", err);
    }
